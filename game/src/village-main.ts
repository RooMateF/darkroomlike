...
function startVillage() {
  ...
  const ADVISOR_BUILDING_HINTS: Record<string, string> = {
    tannery: "她翻著你帶回來的生皮:「這些鞣一鞣就是皮革,做得出很多東西。搭個製革場吧——味道不好聞,選個下風處。」",
    smithy: "她看著你從遺跡帶回的工具,擦了擦上面的鏽:「還能用。搭個工匠鋪,壞掉的傢伙就不用扔了。」",
    "trading-post": "她掂了掂那顆微溫的晶體:「這種東西……有人收,而且出手很闊。蓋個交易的地方,別讓那些人直接進村。」",
    railway: "她望著礦坑的方向:「靠人一趟一趟把礦扛回來,太慢了。鋪一條鐵道,讓台車去跑吧。工程不小,不過值得。」",
  };
  ...
  const REVIVAL_TIPS: Record<string, string[]> = {
    thirst: [
      "她把裝滿的水囊放在你手邊:「水袋變輕的時候,就是該回頭的時候。渴死在半路的人,原本都只差幾步。」",
      "「記住你走過的補給點。回程的水,永遠比去程的重要。」她說完,替你掖了掖毯子。",
    ],
    hunger: [
      "她把一小塊肉乾塞進你手裡:「乾糧見底,不是繼續往前走的理由。」",
      "「別把肉乾當存糧吃光——在外面,那是你最後的血。」她的聲音很輕,但沒有商量的餘地。",
    ],
    combat: [
      "她替你把繃帶收緊:「撐不住就退。武器沒了可以再打一把,人沒了,就什麼都沒了。」",
      "「打不過的東西,就繞開牠。地圖比你以為的寬。」",
      "「多帶一把備用的武器。斷在手裡的柄,救不了你。」",
      "「肉乾要留到戰鬥裡吃。行軍餓了啃乾糧,打起來了才輪到它。」",
    ],
  };
