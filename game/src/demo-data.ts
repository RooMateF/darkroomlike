...
export function buildPlayerCategories(carried: Carried | null): CategoryDef[] {
  ...
  const ranged = carriedWeapons
    ...
    .map((w) => ({ id: w.id, label: w.label, baseCost: w.baseCost, symbol: w.symbol, damage: w.damage, magazine: w.magazine, reloadCost: w.reload, ammoPerUse: w.ammoPerUse, pellets: w.pellets, burst: w.burst }));
