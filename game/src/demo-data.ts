...
export function buildPlayerCategories(carried: Carried | null): CategoryDef[] {
  ...
  const melee = carriedWeapons
    ...
    .map((w) => ({ id: w.id, label: w.label, baseCost: w.baseCost, symbol: w.symbol, damage: w.damage, freeze: w.freeze, stagger: w.stagger, suppress: w.suppress, riposte: w.riposte }));
  const ranged = carriedWeapons
    .filter((w) => w.category === "ranged")
    .map((w) => ({ id: w.id, label: w.label, baseCost: w.baseCost, symbol: w.symbol, damage: w.damage, magazine: w.magazine, reloadCost: w.reload, ammoPerUse: w.ammoPerUse, pellets: w.pellets, burst: w.burst, burstDamages: w.burstDamages }));
  ...
  if ((carried?.coarseBandages ?? 0) > 0) {
    ...
    items.push({ id: "coarse-bandage", label: "粗布繃帶", baseCost: 1.0, symbol: "+", damage: 0, heal: 20, slowReuse: 1.4 });
  }
