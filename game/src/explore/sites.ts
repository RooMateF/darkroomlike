...
export function specialSites(): SpecialSite[] {
  ...
  // 近村 Lv3(2026-09 用戶定案):工匠鋪的門檻(打通首座 Lv3)不該比鐵礦坑(距村 ~23 格)還遠——
  // 在村莊林地圈(半徑 16)內固定一座三層遺跡。放東南側:離教堂(7,5)夠遠,
  // 不會搶走「離教堂最近的 Lv3」的黑鐵鑰匙判定
  {
    let x = 47;
    const y = 38;
    while (taken.has(`${x},${y}`) || landmarkKeys.has(`${x},${y}`) || sites.some((s) => Math.hypot(s.x - x, s.y - y) < 4)) x++;
    taken.add(`${x},${y}`);
    sites.push({ key: `${x},${y}`, x, y, level: 3, stages: 3 });
  }
...
export const SITE_ARRIVAL_TEXT: Record<number, string> = {
  1: "一處還算完整的屋棚,裡面似乎有動靜。看起來不算危險,但不深入不會知道裡面有什麼。",
  2: "有人為整理過的痕跡——這裡的東西還沒被搬空,值得花點時間來探索。",
  3: "殘留的結構比外表看起來深得多。裡面或許比想像中要大得多。",
  4: "這裡散發著危險的氣息……空氣裡有股壓迫感,沒有準備就進去的人,恐怕回不來。",
  5: "空氣沉得像水,連蟲鳴都消失了。或許這裡不是人類可以隨意踏足的禁地。",
};